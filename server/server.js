...
app.get('/setup', async (req, res) => {
  try {
    ...
    db.prepare(`INSERT OR IGNORE INTO users (name, email, password, role) VALUES (?, ?, ?, ?)`).run('Sneha Sharma', 'sneha@gmail.com', hash1, 'admin');
    db.prepare(`INSERT OR IGNORE INTO users (name, email, password, role) VALUES (?, ?, ?, ?)`).run('Team Agent', 'agent@gmail.com', hash2, 'agent');
    res.json({ done: true, message: 'Table created and users added!' });
  } catch(e) {
